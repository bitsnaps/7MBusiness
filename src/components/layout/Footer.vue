<script setup lang="ts">
import { useDarkMode } from '@/composables/useDarkMode';
import { useI18n } from 'vue-i18n'
const { isDark } = useDarkMode()

const { t } = useI18n()
</script>

<template>
  <footer class="relative bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 text-white overflow-hidden">
    <!-- Background decoration -->
    <div class="absolute inset-0 opacity-40">
      <div class="absolute inset-0 bg-gradient-to-br from-blue-500/5 via-purple-500/5 to-indigo-500/5"></div>
      <div class="absolute top-0 left-0 w-full h-full" style="background-image: radial-gradient(circle at 25% 25%, rgba(99, 102, 241, 0.1) 0%, transparent 50%), radial-gradient(circle at 75% 75%, rgba(168, 85, 247, 0.1) 0%, transparent 50%);"></div>
    </div>

    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
        <!-- Company Info -->
        <div class="lg:col-span-5 space-y-6">
          <div class="flex items-center space-x-3">
            <div class="relative">
              <div class="w-12 h-12 flex items-center justify-center shadow-lg">
              <!-- <div class="w-12 h-12 bg-gradient-to-br from-blue-500 via-purple-500 to-indigo-600 rounded-xl flex items-center justify-center shadow-lg"> -->
                <img :src="isDark?'/logo.jpg':'/logo_dark.jpg'" width="48" />
                <!-- <span class="text-white font-bold text-xl">C</span> -->
              </div>
              <div class="absolute -top-1 -right-1 w-4 h-4 bg-gradient-to-r from-emerald-400 to-cyan-400 rounded-full animate-pulse"></div>
            </div>
            <div>
              <h2 class="text-2xl font-bold bg-gradient-to-r from-white to-blue-200 bg-clip-text text-transparent">7M Business</h2>
              <div class="w-8 h-0.5 bg-gradient-to-r from-blue-400 to-purple-400 rounded-full"></div>
            </div>
          </div>

          <p class="text-slate-300 text-lg leading-relaxed max-w-md">
            {{ t('footer.description') }}
          </p>

          <!-- Social Links -->
          <div class="flex space-x-3">
            <a href="#" class="group relative p-3 bg-white/5 hover:bg-white/10 rounded-xl transition-all duration-300 hover:scale-110">
              <svg class="w-5 h-5 text-blue-400 group-hover:text-blue-300 transition-colors" fill="currentColor" viewBox="0 0 24 24">
                <path d="M22.46 6c-.77.35-1.6.58-2.46.69.88-.53 1.56-1.37 1.88-2.38-.83.5-1.75.85-2.72 1.05C18.37 4.5 17.26 4 16 4c-2.35 0-4.27 1.92-4.27 4.29 0 .34.04.67.11.98C8.28 9.09 5.11 7.38 3 4.79c-.37.63-.58 1.37-.58 2.15 0 1.49.75 2.81 1.91 3.56-.71 0-1.37-.2-1.95-.5v.03c0 2.08 1.48 3.82 3.44 4.21a4.22 4.22 0 0 1-1.93.07 4.28 4.28 0 0 0 4 2.98 8.521 8.521 0 0 1-5.33 1.84c-.34 0-.68-.02-1.02-.06C3.44 20.29 5.7 21 8.12 21 16 21 20.33 14.46 20.33 8.79c0-.19 0-.37-.01-.56.84-.6 1.56-1.36 2.14-2.23z"/>
              </svg>
            </a>
            <a href="#" class="group relative p-3 bg-white/5 hover:bg-white/10 rounded-xl transition-all duration-300 hover:scale-110">
              <svg class="w-5 h-5 text-blue-400 group-hover:text-blue-300 transition-colors" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.174-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.957 1.406-5.957s-.359-.72-.359-1.781c0-1.663.967-2.911 2.168-2.911 1.024 0 1.518.769 1.518 1.688 0 1.029-.653 2.567-.992 3.992-.285 1.193.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.063-4.869-5.008-4.869-3.41 0-5.409 2.562-5.409 5.199 0 1.033.394 2.143.889 2.741.097.118.112.221.085.345-.09.375-.293 1.199-.334 1.363-.053.225-.172.271-.402.165-1.495-.69-2.433-2.878-2.433-4.646 0-3.776 2.748-7.252 7.92-7.252 4.158 0 7.392 2.967 7.392 6.923 0 4.135-2.607 7.462-6.233 7.462-1.214 0-2.357-.629-2.746-1.378l-.748 2.853c-.271 1.043-1.002 2.35-1.492 3.146C9.57 23.812 10.763 24.009 12.017 24.009c6.624 0 11.99-5.367 11.99-11.987C24.007 5.367 18.641.001.012.001z"/>
              </svg>
            </a>
          </div>
        </div>

        <!-- Services -->
        <div class="lg:col-span-2">
          <h3 class="text-xl font-bold mb-6 bg-gradient-to-r from-white to-blue-200 bg-clip-text text-transparent">{{ t('footer.services') }}</h3>
          <ul class="space-y-3 list-none">
            <li><a href="#" class="group text-slate-300 hover:text-white transition-all duration-300 hover:translate-x-1 no-underline">
              {{ t('footer.individualCoaching') }}
            </a></li>
            <li><a href="#" class="group text-slate-300 hover:text-white transition-all duration-300 hover:translate-x-1 no-underline">
              {{ t('footer.teamCoaching') }}
            </a></li>
            <li><a href="#" class="group text-slate-300 hover:text-white transition-all duration-300 hover:translate-x-1 no-underline">
              {{ t('footer.businessCoaching') }}
            </a></li>
            <li><a href="#" class="group text-slate-300 hover:text-white transition-all duration-300 hover:translate-x-1 no-underline">
              {{ t('footer.economicCoaching') }}
            </a></li>
            <li><a href="#" class="group text-slate-300 hover:text-white transition-all duration-300 hover:translate-x-1 no-underline">
              {{ t('footer.studentsCoaching') }}
            </a></li>
          </ul>
        </div>

        <!-- Platform -->
        <div class="lg:col-span-2">
          <h3 class="text-xl font-bold mb-6 bg-gradient-to-r from-white to-blue-200 bg-clip-text text-transparent">{{ t('footer.platform') }}</h3>
          <ul class="space-y-3 list-none">
            <li><a href="#" class="group text-slate-300 hover:text-white transition-all duration-300 hover:translate-x-1 no-underline">
              {{ t('footer.aiCoach') }}
            </a></li>
            <li><a href="#" class="group text-slate-300 hover:text-white transition-all duration-300 hover:translate-x-1 no-underline">
              {{ t('footer.dashboard') }}
            </a></li>
            <li><a href="#" class="group text-slate-300 hover:text-white transition-all duration-300 hover:translate-x-1 no-underline">
              {{ t('footer.reports') }}
            </a></li>
            <li><a href="#" class="group text-slate-300 hover:text-white transition-all duration-300 hover:translate-x-1 no-underline">
              {{ t('footer.analytics') }}
            </a></li>
          </ul>
        </div>

        <!-- Company -->
        <div class="lg:col-span-3">
          <h3 class="text-xl font-bold mb-6 bg-gradient-to-r from-white to-blue-200 bg-clip-text text-transparent">{{ t('footer.company') }}</h3>
          <ul class="space-y-3 list-none">
            <li><a href="#" class="group text-slate-300 hover:text-white transition-all duration-300 hover:translate-x-1 no-underline">
              {{ t('footer.aboutUs') }}
            </a></li>
            <li><a href="#" class="group text-slate-300 hover:text-white transition-all duration-300 hover:translate-x-1 no-underline">
              {{ t('footer.contact') }}
            </a></li>
            <li><a href="#" class="group text-slate-300 hover:text-white transition-all duration-300 hover:translate-x-1 no-underline">
              {{ t('footer.privacyPolicy') }}
            </a></li>
            <li><a href="#" class="group text-slate-300 hover:text-white transition-all duration-300 hover:translate-x-1 no-underline">
              {{ t('footer.termsOfService') }}
            </a></li>
          </ul>
        </div>
      </div>

      <!-- Bottom Section -->
      <div class="relative mt-16 pt-8">
        <div class="absolute inset-0 bg-gradient-to-r from-transparent via-blue-500/10 to-transparent"></div>
        <div class="relative border-t border-white/10 pt-8">
          <div class="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <p class="text-slate-400 text-sm">
              © {{ new Date().getFullYear() }} {{ t('header.brand') }}. All rights reserved.
            </p>
            <div class="flex items-center space-x-6 text-sm text-slate-400">
              <span class="flex items-center space-x-2">
                <div class="w-2 h-2 bg-emerald-400 rounded-full animate-pulse"></div>
                <span>{{ t('header.allSystemsOperational') }}</span>
              </span>
            </div>
          </div>
        </div>
      </div>
    </div>
  </footer>
</template>